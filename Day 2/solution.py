...
modified_list = list(divide_chunks(given_list, 3))
# print(modified_list)
...
for i in range(0, len(modified_list)):
    min_value.append(modified_list[i][0].split("-")[0])
    max_value.append(modified_list[i][0].split("-")[1])
    letter.append(modified_list[i][1].replace(':', ''))
    password.append(modified_list[i][2])

# # first problem

for i in range(0,len(modified_list)):
  if letter[i] in password[i]:
    count=Counter(password[i])
    if count[letter[i]] >= int(min_value[i]) and count[letter[i]] <= int(max_value[i]):
      valid_passwords+=1
...
correct_passwords = 0
for i in range(0, len(modified_list)):
    if (password[i][int(min_value[i]) - 1] == letter[i] and password[i][int(max_value[i]) - 1] != letter[i]) or (
            password[i][int(min_value[i]) - 1] != letter[i] and password[i][int(max_value[i]) - 1] == letter[i]):
        correct_passwords += 1
